...
APP_VERSION = "Pattern OCR Translator (Beta RC22d)"
...
def _find_free_label_position(
    ...
    # RC22d POC: favor row association over distant empty geometry.
    # Keep labels near their source row; if no local placement works, marker
    # fallback preserves the row association more clearly than a distant label.
    start_x = max_x + 6
    max_search_x = min(image_w - tw - 4, max(start_x, image_w * 0.72))
    row_height = max(1.0, max_y - min_y)
    local_offsets = [0, min(row_height * 0.45, 8.0), min(row_height, 14.0)]
