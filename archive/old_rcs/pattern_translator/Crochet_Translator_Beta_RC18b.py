...
APP_VERSION = "Pattern OCR Translator (Beta RC18b)"
...
def _find_free_label_position(
    ...
    # For very wide OCR boxes near the right edge, a clamped "right" label can look
    # like it sits on top of its own text. Try the left side first only in that
    # cramped edge case; other OCR boxes remain protected.
    right_space = image_w - max_x - 6
    right_edge_cramped = max_x > image_w * 0.78 and right_space < tw + 12
    if right_edge_cramped:
        for y in [min_y, min_y + th * 0.35]:
            pos = safe_at(min_x - tw - 6, y)
            if pos is not None:
                return pos
...
def _find_free_marker_position(
    ...
    # Last resort: keep the marker tiny and nearby, but avoid the left edge of the OCR text.
    x = max(4, min(max_x + 4, image_w - mw - 4))
    y = max(4, min(min_y, image_h - mh - 4))
    rect = (x, y, x + mw, y + mh)
    for _ in range(8):
        if not any(_rects_overlap(rect, r) for r in used_slots):
            break
        y = min(image_h - mh - 4, y + mh + 2)
        rect = (x, y, x + mw, y + mh)
    return x, y, rect
    ...
    for row_no, (_, row) in enumerate(line_df.iterrows()):
        ...
        if not placed_full:
            ...
            x, y, rect = _find_free_marker_position(
                min_x, min_y, max_x, max_y, mw, mh, w, h, used_slots, protected_slots + [current_source_slot]
            )
