...
APP_VERSION = "Pattern OCR Translator (Beta RC22a)"
...
def _find_free_marker_position(
    ...
    # Last resort: keep the marker tiny and nearby while still respecting OCR protection.
    ...
    for _ in range(12):
        ...
        rect = (x, y, x + mw, y + mh)
    if any(_rects_overlap(rect, r) for r in protected_slots):
        scan_step = max(6.0, mh + 2)
        y_scan = max(4.0, min_y)
        while y_scan <= image_h - mh - 4:
            candidate = (x, y_scan, x + mw, y_scan + mh)
            if not any(_rects_overlap(candidate, r) for r in protected_slots):
                return x, y_scan, candidate
            y_scan += scan_step
        y_scan = max(4.0, min_y - scan_step)
        while y_scan >= 4.0:
            candidate = (x, y_scan, x + mw, y_scan + mh)
            if not any(_rects_overlap(candidate, r) for r in protected_slots):
                return x, y_scan, candidate
...
def make_line_translation_overlay(
    ...
    for row_no, (_, row) in enumerate(line_df.iterrows()):
        ...
        current_row_marker_slot = _left_row_marker_protected_slot(original, min_x, min_y, max_x, max_y)
