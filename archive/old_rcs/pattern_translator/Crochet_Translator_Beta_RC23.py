...
APP_VERSION = "Pattern OCR Translator (Beta RC23)"
...
FOUNDATION_CHAIN_INSTRUCTION_RE = re.compile(
    r"(?<![環环圈])(?:起|立)\s*(?P<number>\d+|[一二三四五六七八九十]+)\s*(?:个|個)?\s*(?:辮子針|辫子针|鎖針|锁针|CH|ch|chain)"
)
...
def translate_expression(expr: str, index: Dict[str, int], df: pd.DataFrame, output_mode: str = "Traditional Chinese") -> str:
    ...
    chain_start_recovered = replace_foundation_chain_instructions(original, output_mode, protect=protect_chain_start_term)
