...
APP_VERSION = "Pattern OCR Translator (Beta RC22b)"
...
def _left_row_marker_protected_slot(
    original: str,
    min_x: float,
    min_y: float,
    max_x: float,
    ...
    """Protect the row/round number at the left edge of the current OCR row."""
    text = unicodedata.normalize("NFKC", str(original or "")).strip()
    if not text:
        return None
    m = re.match(
        r"^\s*((?:Rnd\s*)?R?\s*[0-9lIgq]+(?:\s*[-–—~～〜－]\s*R?\s*\d+)?\s*[:：.]?|\d+\s*[.．、])",
        text,
        flags=re.I,
    )
    if not m:
    ...
    prefix_chars = len(m.group(1))
    total_chars = max(prefix_chars, len(text))
    estimated_width = row_width * min(0.55, max(0.12, (prefix_chars + 0.5) / total_chars))
    protected_width = min(row_width, max(28.0, estimated_width + 4.0))
    return _pad_rect((min_x, min_y, min_x + protected_width, max_y), 2)


def _looks_like_section_heading_anchor(text: str) -> bool:
    """Heuristic for short section headings without maintaining a title dictionary."""
    ...
    if not compact or len(compact) > 18:
        return False
    if _left_row_marker_protected_slot(text, 0, 0, 80, 20) is not None:
        return False
    ...
    if re.search(r"\b(?:sc|dc|tr|hdc|slst|inc|dec|ch|mr|blo|flo|rnd|round|row)\b", lower_text):
        return False
    if re.search(r"[xvawmtfesl]", lower_text) and not re.fullmatch(r"[A-Za-z ]+", text):
        return False
    if re.search(r"[針针鉤钩鎖锁辮辫圈环]", text):
        return False

    cjk_chars = re.findall(r"[\u3400-\u9fff]", compact)
    if cjk_chars and len(cjk_chars) == len(compact) and 1 <= len(cjk_chars) <= 8:
        return True

    words = re.findall(r"[A-Za-z]+", text)
    if 1 <= len(words) <= 3 and sum(len(w) for w in words) <= 16:
        return all(w[:1].isupper() or w.isupper() for w in words)
    return False


def _left_reading_anchor_protected_slot(
    original: str,
    min_x: float,
    min_y: float,
    max_x: float,
    ...
    """Protect row labels and short heading anchors at the left edge of the current row."""
    row_slot = _left_row_marker_protected_slot(original, min_x, min_y, max_x, max_y)
    if row_slot is not None:
        return row_slot
    if not _looks_like_section_heading_anchor(original):
        return None
    row_width = max(1.0, float(max_x) - float(min_x))
    protected_width = min(row_width, max(28.0, min(76.0, row_width * 0.42)))
    return _pad_rect((min_x, min_y, min_x + protected_width, max_y), 2)
...
def make_line_translation_overlay(
    ...
    for row_no, (_, row) in enumerate(line_df.iterrows()):
        ...
        current_row_marker_slot = _left_reading_anchor_protected_slot(original, min_x, min_y, max_x, max_y)
