...
def format_group_with_stitch_count(inner: str, count: str, index: Dict[str, int], df: pd.DataFrame, output_mode: str) -> str:
    parts = [translate_piece(part, index, df, output_mode) for part in split_expression_parts(inner)]
...
def replace_csv_terms_in_line(text: str, index: Dict[str, int], df: pd.DataFrame, output_mode: str) -> str:
    ...
    counted_pat = re.compile(
        r"(?<![A-Za-z0-9])(\d+)\s*(sl\s*st|slst|sts?|stitches?|sc|inc|dec|hdc|dc|tr|mr|ch|blo|flo|fo|[XVATFEWM])\b",
        flags=re.I,
    )
    ...
    def counted_repl(m: re.Match) -> str:
        ...
        # st/sts/stitch/stitches are counts, not a specific stitch.
        if norm_text(term_clean) in {"st", "sts", "stitch", "stitches"}:
            if output_mode == "Simplified Chinese":
...
def translate_piece(piece: str, index: Dict[str, int], df: pd.DataFrame, output_mode: str = "Traditional Chinese") -> str:
    ...
    if m:
        ...
        parts = [translate_piece(part, index, df, output_mode) for part in split_expression_parts(inside)]
    ...
    m = re.fullmatch(r"\((.*?)\)", p)
    if m:
        parts = [translate_piece(part, index, df, output_mode) for part in split_expression_parts(m.group(1))]
    ...
    # Chinese-mainland shorthand: 2X / 10x / 1V / 6A
...
def translate_expression(expr: str, index: Dict[str, int], df: pd.DataFrame, output_mode: str = "Traditional Chinese") -> str:
    ...
    if m:
        ...
        parts = [translate_piece(p, index, df, output_mode) for p in split_expression_parts(inside)]
    ...
    if m:
        parts = [translate_piece(p, index, df, output_mode) for p in split_expression_parts(m.group(1))]
...
def _find_free_label_position(
    ...
    """Find a nearby label position that does not collide with previous labels."""
    candidates = [
        (max_x + 6, min_y),          # right
        (min_x, max_y + 4),          # below
        (min_x, min_y - th - 4),     # above
        (min_x - tw - 6, min_y),     # left
    ...
    for x, y in candidates:
        ...
        rect = (x, y, x + tw, y + th)
        if not any(_rects_overlap(rect, r) for r in used_slots):
            return x, y
    return None
...
def make_line_translation_overlay(
    ...
    overlay = Image.new("RGBA", img.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    font_size = max(13, min(24, int(w / 55)))
    font = _load_overlay_font(font_size)
    marker_font = _load_overlay_font(max(font_size, 15))
    ...
    for _, row in line_df.iterrows():
        ...
        if not force_marker:
            lines = _wrap_label(label, max_chars=24)
            bboxes = [draw.textbbox((0, 0), line, font=font) for line in lines]
            tw = max(bb[2] - bb[0] for bb in bboxes) + 14
            th = sum(bb[3] - bb[1] for bb in bboxes) + 10 + (len(lines)-1)*3
            pos = _find_free_label_position(min_x, min_y, max_x, max_y, tw, th, w, h, used_slots)
            if pos is not None:
                x, y = pos
                used_slots.append((x, y, x + tw, y + th))
        ...
        if not placed_full:
            ...
            mw = mbb[2] - mbb[0] + 12
            mh = mbb[3] - mbb[1] + 8
            # Put marker at the OCR box itself; it is deliberately small.
            x = max(4, min(min_x, w - mw - 4))
            y = max(4, min(min_y, h - mh - 4))
            rect = (x, y, x + mw, y + mh)
            # If markers collide, nudge down a few times. If still colliding, draw anyway.
            for _ in range(8):
                if not any(_rects_overlap(rect, r) for r in used_slots):
                    break
                y = min(h - mh - 4, y + mh + 2)
...
INTERFACE_LANGUAGES = {
    "English": {
        ...
        "camera_prompt": "Take a photo of the pattern",
        "original_image": "Original image",
        "translation_area": "Translation area",
        "translation_area_tip": "💡 For the best experience, select only the area you want to translate whenever possible. It is usually faster and more accurate than translating the whole pattern.",
    ...
    "繁體中文": {
        ...
        "image_source_label": "圖片來源",
        "upload_image": "上載圖片",
        "take_photo": "使用相機拍照",
        "upload_prompt": "上載圖樣圖片",
        "camera_prompt": "拍攝圖樣照片",
        "original_image": "原始圖片",
        "translation_area": "翻譯範圍",
        "translation_area_tip": "💡 建議只選取需要翻譯的範圍，通常會更快、更準確，也能提升整體使用體驗。",
        "area_label": "要翻譯的範圍",
        "area_select": "選取範圍",
        "area_left": "左欄",
        "area_right": "右欄",
    ...
    "简体中文": {
        ...
        "image_source_label": "图片来源",
        "upload_image": "上传图片",
        "take_photo": "使用相机拍照",
        "upload_prompt": "上传图样图片",
        "camera_prompt": "拍摄图样照片",
        "original_image": "原始图片",
        "translation_area": "翻译范围",
        "translation_area_tip": "💡 建议只选择需要翻译的区域，通常会更快、更准确，也能提升整体使用体验。",
        "area_label": "要翻译的范围",
        "area_select": "选取范围",
        "area_left": "左栏",
        "area_right": "右栏",
    ...
    "日本語": {
        ...
        "image_source_label": "画像の入力方法",
        "upload_image": "画像をアップロード",
        "take_photo": "カメラで撮影",
        "upload_prompt": "パターン画像をアップロード",
        "camera_prompt": "パターンを撮影",
        "original_image": "元の画像",
        "translation_area": "翻訳する範囲",
        "translation_area_tip": "💡 必要な部分だけを選択すると、通常はより速く、より正確に翻訳できます。",
        "area_label": "翻訳する範囲",
        "area_select": "範囲を選択",
        "area_left": "左の列",
        "area_right": "右の列",
...
IMAGE_SOURCE_LABEL_KEYS = {
    "Upload image": "upload_image",
    "Take photo with camera": "take_photo",
}
...
input_mode = st.segmented_control(
    t("image_source_label"),
    ["Upload image", "Take photo with camera"],
    default="Upload image",
    key="image_source_mode_radio",
    label_visibility="collapsed",
    format_func=lambda value: t(IMAGE_SOURCE_LABEL_KEYS.get(value, value)),
    width="stretch",
)
...
if input_mode == "Upload image":
    ...
    image_file = st.camera_input(t("camera_prompt"))
...
if image_file is not None:
    ...
    if area_mode == "Whole Pattern":
        ...
        adjust_mode = "Drag rectangle on image"
        ...
        if cropper_ok:
            ...
            try:
                ...
                cropper_box = crop_box_from_cropper_result(cropper_result, image)
                if cropper_box is not None:
        ...
        if adjust_mode != "Drag rectangle on image":
            w, h = image.size
            left0, top0, right0, bottom0 = preset_box
            st.caption(t("boundary_instruction"))
            col_a, col_b = st.columns(2)
            safe_area_mode = re.sub(r"\W+", "_", area_mode_for_box.lower())
            key_suffix = f"{current_signature[:12]}_{safe_area_mode}"
            with col_a:
            ...
            crop_box = clamp_crop_box((left, top, right, bottom), image)
            st.session_state["latest_crop_box"] = crop_box

        latest_crop_box = st.session_state.get("latest_crop_box")
        if latest_crop_box is None:
            latest_crop_box = crop_box
            st.session_state["latest_crop_box"] = latest_crop_box
        crop_box = clamp_crop_box(latest_crop_box, image)
    ...
    st.button(
        ...
        type="primary",
        disabled=ocr_running or (bool(quality_errors) and not force_run),
