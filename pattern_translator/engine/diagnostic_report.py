...
def build_debug_report_text(
    ...
    parts = [
        ...
        f"Assessment resolution: {quality_metrics.get('assessment_width_px', '')} x {quality_metrics.get('assessment_height_px', '')} px",
        f"Sharpness: {quality_metrics.get('sharpness_score', '')}",
        f"Contrast: {quality_metrics.get('contrast_score', '')}",
        f"Plausible text-like components: {quality_metrics.get('plausible_component_count', '')}",
        f"Main text height (H_main): {quality_metrics.get('main_text_height_px', '')}",
        f"Main text height reliable: {'Yes' if quality_metrics.get('main_text_height_reliable') is True else 'No'}",
        f"Dominant height band: {quality_metrics.get('dominant_band_component_count', '')} components ({quality_metrics.get('dominant_band_component_percent', '')}%)",
        f"Image quality classification reason: {quality_metrics.get('classification_reason', '')}",
        "",
        "=== OCR Image Pipeline ===",
